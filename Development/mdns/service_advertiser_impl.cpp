        ...
        public:
            ...
            pplx::task<bool> register_address(const std::string& host_name, const std::string& ip_address, const std::string& domain, std::uint32_t interface_id) override
            {
                return pplx::create_task([=]
                {
                    ...
                    int state = avahi_client_get_state(pContext->client);
                    if (AVAHI_CLIENT_S_RUNNING == state)
                    {
                        avahi_threaded_poll_lock(pContext->threaded_poll);
                        client_callback(pContext->client, avahi_client_get_state(pContext->client), pContext);
                        avahi_threaded_poll_unlock(pContext->threaded_poll);
                    }
